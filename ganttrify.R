# giocomai/ganttrify is licensed under the
# GNU General Public License v3.0
#
# Permissions of this strong copyleft license are conditioned on making available complete source code of licensed works and modifications, which include larger works using a licensed work, under the same license. Copyright and license notices must be preserved. Contributors provide an express grant of patent rights.
#
# For more information see https://github.com/giocomai/ganttrify
# 
# Modified by Tony Alexander Hild (2021)
#
#
#
#
...
ganttrify <- function(project,
                      ...
                      colour_palette = gg_color_hue(15),
                      font_family = "sans",
                      mark_quarters = FALSE,
                      mark_years = FALSE,
                      size_wp = 6,
  ...
  if (length(unique(project$wp))>length(as.character(gg_color_hue(15)))) {
  ...
  if (month_number_label==TRUE&month_date_label==TRUE) {
    gg_gantt <- gg_gantt +
      ggplot2::scale_x_date(name = "", 
                            ...
                            date_labels = "%m\n%y",
                            minor_breaks = NULL,
                            sec.axis = ggplot2::dup_axis(name = "Meses do período de execução do projeto", labels = paste0("M", seq_along(date_breaks)*month_breaks-(month_breaks-1))))
  ...
  gg_gantt <- suppressWarnings(gg_gantt 
                            ...
                            #   + ggplot2::scale_y_discrete(name="Metas e/ou atividades", labels=labels.wrap)
                               ...
                               + ggplot2::theme(
                                 ...
                                 axis.text.y.left = ggplot2::element_text(face = ifelse(test = df_yearmon_fct %>% dplyr::distinct(activity, wp, type) %>% dplyr::pull(type)=="wp", yes = "bold", no = "plain"), hjust = axis_text_align_n),
